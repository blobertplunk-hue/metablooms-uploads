...
class SEE(StageBase):
    ...
    def execute(self, state: Any) -> dict[str, Any]:
        ...
        # FIX: enforce full SIE coverage at Stage 1, not Stage 2
        covered = {r.get("dimension") for r in validated if r.get("dimension")}
        missing_dims = REQUIRED_COVERAGE_DIMS - covered
        if missing_dims:
            raise RuntimeError(
                f"SEE_COVERAGE_INCOMPLETE: missing dimensions {sorted(missing_dims)}. "
                f"All three dimensions (theory, implementation, failure_modes) are required."
            )
...
class CDR(StageBase):
    ...
    def execute(self, state: Any) -> dict[str, Any]:
        ...
        # POLICY_ENGINE centralized checks first
        for policy, ctx_keys in [
            ("cdr_attestation", {"attestation": constraints.get("attestation")}),
            ("module_naming", {"module_name": constraints.get("module_name", "")}),
        ]:
            decision = evaluate_policy(policy, ctx_keys)
            if not decision.allowed:
                raise RuntimeError(f"POLICY_DENY [{policy}]: {decision.violations}")
...
class VERIFICATION(StageBase):
    ...
    def execute(self, state: Any) -> dict[str, Any]:
        ...
        # Check 1b: POLICY_ENGINE — check stages that must precede VERIFICATION
        stages_ran = list(state.data.keys())
        pre_verif_mandatory = {"SEE", "CDR", "IMPLEMENTATION"}
        missing_pre = pre_verif_mandatory - set(stages_ran)
        if missing_pre:
            failures.append(f"POLICY_DENY [pre_verification]: mandatory stages missing: {sorted(missing_pre)}")
