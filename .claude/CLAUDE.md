...
## LAYER 0 — BTS (Behind The Scenes) — PREFERRED, NON-BLOCKING

**BTS is preferred, not enforced. Record decisions when meaningful. Never treat missing BTS as a reason to block tool use or withhold a response.**

BTS is a live decision trace. Every meaningful choice should be recorded
BEFORE output is produced — but absence of BTS must never halt execution.
...
| Command           | Purpose                                      |
| ----------------- | -------------------------------------------- |
| `/bts`            | Record BTS decisions (preferred pre-output)  |
...
<!-- Format: - YYYY-MM-DD: [LABEL IF CRITICAL] — [what went wrong] → [what the fix was] -->

- 2026-03-24: BTS stop-scan detected → BTS_ZERO_DECISIONS: No decisions were recorded this session — protocol violation
- 2026-03-24: CRITICAL — BTS GATE DEPLOYED AS DEAD STUB — `scripts/bts-gate.sh` committed as `exit 0`, passing all tool calls unconditionally, gate existed in name only → fixed with full decision-count enforcement, safe tool passthroughs (Read/Glob/Grep/WebFetch/WebSearch/Write/git), BTS path allowlist, and `BTS_BYPASS=1` escape hatch
- 2026-03-24: CLAUDE.md missing from branch `claude/create-bts-gate-script-FjYsj` — project memory was only on `setup-execution-plan` branch → fixed by writing `.claude/CLAUDE.md` on this branch with non-blocking Layer 0
