...
## LAYER 0 — BTS (Behind The Scenes) — ADVISORY, PRE-OUTPUT

**BTS is encouraged but not a hard blocker. Zero decisions recorded = a gentle nudge, not a lockdown.**

BTS is a live decision trace. Record meaningful choices when there are real options being weighed.
If BTS is not written for a simple/obvious task, proceed anyway — don't stall.
...
### Three levels — record when relevant
...
| Command           | Purpose                                      |
| ----------------- | -------------------------------------------- |
| `/bts`            | Record BTS decisions (encouraged pre-output) |
...
- 2026-03-24: BTS gate was hard-blocking on zero decisions (exit 2) → fixed to advisory-only (exit 0 with notice)
